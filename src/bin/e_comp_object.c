...
e_comp_object_util_fullscreen(Evas_Object *obj)
{
   ...
   if (cw)
     {
//        L("HINT: 0x%x fullscreen copy object", (int)e_client_util_win_get(cw->ec));
...
//  L("SHP: e_comp_object_input_area_set %p %p", obj, area);
...
//  L("SHP: ZZZZZZZZZ add input rect to %p", obj);
...
//          L("SHP: XXXXX [%p] [%s]", cw->obj, evas_object_type_get(cw->obj));
...
//              L("SHP: XXXXX %i %i %ix%i", rects[i].x, rects[i].y, rects[i].w, rects[i].h);
            }
          rects[i].x = 0;
          rects[i].y = 0;
          rects[i].w = 0;
    ...
    {
//      L("SHP: XXXXX [%p] [%s] RESET!!!!!", cw->obj, evas_object_type_get(cw->obj));
